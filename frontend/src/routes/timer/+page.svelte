...
<div class="scroll-area fade-in">
    <div class="header-actions">
        <h2>Focus Timer</h2>
        <div class="phase-badge" style="background: {isWorkPhase ? 'rgba(99, 102, 241, 0.1)' : 'rgba(34, 197, 94, 0.1)'}; border-color: {accentColor}; color: {accentColor}">
            <i class={isWorkPhase ? 'bx bxs-brain' : 'bx bxs-coffee'}></i> {currentPhaseText}
        </div>
    </div>
    ...
    <div class="timer-container">
        ...
        <div class="mode-selector">
            <button class="mode-btn {mode === 'pomodoro' ? 'active' : ''}" on:click={() => switchMode('pomodoro')}>Pomodoro (25/5)</button>
            <button class="mode-btn {mode === '5217' ? 'active' : ''}" on:click={() => switchMode('5217')}>52/17 Rule</button>
            <button class="mode-btn {mode === 'custom' ? 'active' : ''}" on:click={() => switchMode('custom')}>Custom</button>
        </div>
        ...
        {#if mode === 'custom'}
            <div class="custom-inputs slide-down">
                <div class="input-group">
                    ...
                    <input type="number" id="work-min" bind:value={customWorkMin} on:change={updateCustomTime} min="1">
                </div>
                <div class="input-group">
                    <label for="break-min">Break (min)</label>
                    <input type="number" id="break-min" bind:value={customBreakMin} on:change={updateCustomTime} min="1">
                </div>
            </div>
        {/if}
        ...
        <div class="controls">
            ...
            <button class="control-btn primary lg" on:click={toggleTimer} style="background-color: {accentColor}">
                {#if isRunning}
...
<style>
    ...
    .phase-badge { 
        padding: 0.5rem 1rem; border-radius: 2rem; border: 1px solid; 
        font-size: 0.85rem; font-weight: 600; display: flex; align-items: center; gap: 0.5rem; transition: all 0.3s ease;
    }
    ...
    .input-group label { font-size: 0.7rem; color: var(--text-gray); text-transform: uppercase; margin-bottom: 0.5rem; font-weight: 600; }
    .input-group input {
        background: #0B0C15; border: 1px solid var(--border-color); color: white;
        padding: 0.5rem; border-radius: 0.5rem; width: 4rem; text-align: center;
    ...
    .top .num { top: 0; }
    .bottom .num, .bottom-flap .num { bottom: 0; } /* Keeps the text anchored to the bottom */
    ...
    @keyframes dropDown {
        ...
        0% { transform: rotateX(-90deg); filter: brightness(1.5); }
    ...
    .control-btn.secondary {
        width: 3.5rem; height: 3.5rem; background: #1E1F2E; color: var(--text-gray); border: 1px solid var(--border-color);
    }
    .control-btn.secondary:hover { color: white; border-color: #4b5563; }
    
    .control-btn.primary.lg {
        width: 5rem; height: 5rem; border-radius: 1.5rem; color: white; font-weight: bold; font-size: 1.1rem; gap: 0.5rem; flex-direction: column;
        box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.3);
    }
    .control-btn.primary.lg i { font-size: 2rem; margin-bottom: -0.2rem; }
    .control-btn.primary.lg:hover { filter: brightness(1.1); box-shadow: 0 15px 20px -3px rgba(0, 0, 0, 0.4); }
    ...
    @media (max-width: 600px) {
        .flip-card { width: 90px; height: 110px; font-size: 4rem; }
        .num { height: 110px; } 
        .colon { font-size: 3rem; }
        .timer-container { padding: 2rem 1rem; border-radius: 0; border-left: none; border-right: none; }
    }
</style>